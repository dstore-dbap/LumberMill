...
class TestTcpServerThreaded(ModuleBaseTestCase.ModuleBaseTestCase):
    ...
    def testATlsTcpConnection(self):
        ...
        try:
            ...
            connection_succeeded = False
        self.assertTrue(connection_succeeded)
        expected_ret_val =  Utils.getDefaultEventDict({'gambolputty': {'id': 1}, 'received_from': '127.0.0.1', 'data': "Beethoven, Mozart, Chopin, Liszt, Brahms, Panties...I'm sorry...Schumann, Schubert, Mendelssohn and Bach. Names that will live for ever."})
        self.test_object.shutDown()
        # Give server some time to shut socket down.
        time.sleep(.1)
        for event in self.receiver.getEvent():
            event['gambolputty']['id'] = 1
            self.assertEquals(event, expected_ret_val)