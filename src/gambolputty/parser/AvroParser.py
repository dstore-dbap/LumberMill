...
class AvroParser(BaseModule.BaseModule):
    ...
    - module: AvroParser
      ...
      schema:                                 # <type: string; is: required>
    ...
    def configure(self, configuration):
        ...
        try:
            self.schema = avro.schema.parse(open(self.getConfigurationValue('schema')))
        except:
            etype, evalue, etb = sys.exc_info()
            self.logger.error("%sCould not read avro schema: %s. Exception: %s, Error: %s.%s" % (Utils.AnsiColors.FAIL, self.getConfigurationValue('schema'), etype, evalue, Utils.AnsiColors.ENDC))
            self.gp.shutDown()
            return
    ...
    def decodeEvent(self, event):
        for source_field in self.source_fields:
            ...
            try:
                ...
                etype, evalue, etb = sys.exc_info()
                self.logger.error("%sCould not parse msgpack event data: %s. Exception: %s, Error: %s.%s" % (Utils.AnsiColors.WARNING, event, etype, evalue, Utils.AnsiColors.ENDC))
                continue
